...
router.post("/checkout/quote", requireAuth, requireRole(["customer"]), (req: AuthedRequest, res) => {
  const restaurantId = String(req.body?.restaurantId || "");
  const items = Array.isArray(req.body?.items) ? req.body.items : [];
  let subtotal = 0;
  for (const item of items) {
    const menuItem = getRestaurantMenuItem(restaurantId, String(item.menuItemId));
    if (!menuItem) {
      continue;
    }
...
router.post("/checkout/place-order", requireAuth, requireRole(["customer"]), (req: AuthedRequest, res) => {
  ...
  if (orderRepository.isEnabled()) {
    return (async () => {
      ...
      if (idempotencyRepository.isEnabled()) {
        ...
        const cached = await idempotencyRepository.find(req.session!.userId, "checkout_place_order", idempotencyKey);
        if (cached) {
      ...
      const restaurant = await catalogRepository.getRestaurantById(payload.restaurantId);
      if (!restaurant) {
        return sendApiError(res, 404, "NOT_FOUND", "Restaurant not found");
      }
      const menu = (await catalogRepository.getMenuByRestaurant(restaurant.id)) || [];
      const address = pgUser.addresses.find((a) => a.id === payload.deliveryAddressId) || pgUser.addresses[0];
      if (!address) {
        return sendApiError(res, 400, "VALIDATION_ERROR", "Delivery address required", { field: "deliveryAddressId" });
      }
      let subtotal = 0;
      ...
      if (!created) {
        return sendApiError(res, 500, "INTERNAL_ERROR", "Unable to create order");
      }
      ...
      if (idempotencyRepository.isEnabled()) {
        await idempotencyRepository.save(req.session!.userId, "checkout_place_order", idempotencyKey, response, 200);
      }
  ...
  const address = user.addresses.find((a) => a.id === payload.deliveryAddressId) || user.addresses[0];
  if (!address) {
    return sendApiError(res, 400, "VALIDATION_ERROR", "Delivery address required", { field: "deliveryAddressId" });
  }
  let subtotal = 0;
...
router.post("/orders", requireAuth, requireRole(["customer"]), (req: AuthedRequest, res) => {
  if (orderRepository.isEnabled()) {
    return (async () => {
      const pgUser = await authRepository.getUserById(req.session!.userId);
      if (!pgUser) {
      ...
      const restaurantId = String(req.body?.restaurantId || "");
      const restaurant = await catalogRepository.getRestaurantById(restaurantId);
      if (!restaurant) {
        return res.status(404).json({ success: false, message: "Restaurant not found" });
      }
      const menu = (await catalogRepository.getMenuByRestaurant(restaurant.id)) || [];
      const deliveryAddress =
        pgUser.addresses.find((a) => a.id === req.body?.deliveryAddressId) || pgUser.addresses[0];
      if (!deliveryAddress) {
      ...
      const items = itemsReq
        .map((raw: any) => {
          const menuItem = menu.find((item) => item.id === String(raw.menuItemId));
          if (!menuItem) {
            return undefined;
          }
  ...
  const deliveryAddress = user.addresses.find((a) => a.id === req.body?.deliveryAddressId) || user.addresses[0];
  if (!deliveryAddress) {
    return res.status(400).json({ success: false, message: "Delivery address required" });
  }
  const itemsReq = Array.isArray(req.body?.items) ? req.body.items : [];
  ...
  const items = itemsReq
    .map((raw: any) => {
      const menuItem = getRestaurantMenuItem(restaurant.id, String(raw.menuItemId));
      if (!menuItem) {
        return undefined;
      }
      const quantity = Number(raw.quantity || 1);
...
router.get("/orders", requireAuth, requireRole(["customer"]), (req: AuthedRequest, res) => {
  if (orderRepository.isEnabled()) {
    return (async () => {
      const status = typeof req.query.status === "string" ? req.query.status : undefined;
      const orders = await orderRepository.getOrdersByUser(req.session!.userId, status);
      if (!orders) {
...
router.get("/orders/active", requireAuth, requireRole(["customer"]), (req: AuthedRequest, res) => {
  if (orderRepository.isEnabled()) {
    return (async () => {
      const orders = await orderRepository.getActiveOrdersByUser(req.session!.userId);
      if (!orders) {
  ...
  const orders = Array.from(db.orders.values()).filter((order) => {
    return order.userId === req.session!.userId && !["delivered", "cancelled", "refunded"].includes(order.status);
  });
  res.json({ orders });
});
...
router.post("/orders/:orderId/cancel", requireAuth, (req, res) => {
  ...
  events.push({ id: util.id("evt"), status: "cancelled", timestamp: util.nowIso(), message: "Order cancelled" });
...
router.get("/orders/:orderId/status", requireAuth, (req, res) => {
  if (orderRepository.isEnabled()) {
    return (async () => {
      ...
      return res.json({ status: order.status, estimatedDelivery: order.estimatedDelivery });
    })();
  }
...
router.post("/orders/:orderId/subscribe-notifications", requireAuth, (_req, res) => {
  res.json({ success: true });
});

router.delete("/orders/:orderId/subscribe-notifications", requireAuth, (_req, res) => {
  res.json({ success: true });
});
