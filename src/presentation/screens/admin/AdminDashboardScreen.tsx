...
import {
  ...
  Seller,
  User,
} from '../../../data/api/adminUserService';
import {
  adminPayoutService,
  PayoutItem,
} from '../../../data/api/adminPayoutService';
import {
  adminAuditService,
  AuditLogItem,
} from '../../../data/api/adminAuditService';
import {
  dispatchService,
  DispatchOrder,
  DispatchRider,
  ...
  const handleSellerToggle = async (seller: Seller) => {
    await withAction(`seller-${seller.id}`, async () => {
      ...
      if (!res.success) {
        ...
        Alert.alert(
          'Action Failed',
          res.error || 'Unable to reactivate seller',
        );
      } else {
        await logAudit({
          action: 'reactivate_seller',
  ...
  const handlePayoutAction = async (
    ...
    await withAction(`payout-${payout.id}-${action}`, async () => {
      if (action === 'processing') {
        ...
        if (!res.success) {
          ...
          Alert.alert(
            ...
            res.error || 'Unable to mark processing',
          );
        } else {
          await logAudit({
            action: 'payout_mark_processing',
      ...
      if (!res.success) {
        ...
        Alert.alert(
          'Payout Action Failed',
          res.error || 'Unable to hold payout',
        );
      } else {
        await logAudit({
          action: 'payout_hold',
  ...
  const handleAssignRider = async (order: DispatchOrder) => {
    ...
    await withAction(`dispatch-assign-${order.id}`, async () => {
      ...
      if (!result.success) {
        ...
        Alert.alert(
          'Dispatch Failed',
          result.error || 'Unable to assign rider',
        );
        return false;
      }
      await logAudit({
    ...
    await withAction(`dispatch-progress-${order.id}-${next.next}`, async () => {
      ...
      if (result.order?.proofOtp) {
        Alert.alert(
          ...
          `Proof OTP: ${result.order.proofOtp}`,
        );
      }
      return true;
    });
