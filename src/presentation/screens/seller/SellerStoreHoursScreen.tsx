...
export const SellerStoreHoursScreen: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (closedDays.length === 7) {
      Alert.alert('Invalid', 'At least one day should be open.');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        Alert.alert('Success', 'Store hours have been updated.', [
          { text: 'OK', onPress: () => navigation.goBack() },
        ]);
      } else {
        Alert.alert('Error', result.error || 'Failed to update store hours');
      }
    } catch {
