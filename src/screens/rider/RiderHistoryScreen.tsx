...
import { riderService, RiderOrder } from '../../api/riderService';
...
export const RiderHistoryScreen: React.FC = () => {
      ...
      <View style={styles.orderHeader}>
        ...
        >
          <Text style={[styles.statusText, { color: colors.success }]}>
            {item.status === 'delivered' ? 'âœ“ Delivered' : item.status}
          </Text>
        </View>
      </View>
  ...
  return (
    ...
    >
      ...
      {/* Orders List */}
      {((loading && orders.length === 0) || !minDelayDone) ? (
        <View style={[styles.loadingContainer, { paddingHorizontal: 16, justifyContent: 'flex-start' }]}>
          {Array.from({ length: 3 }).map((_, index) => (
            <View key={`history-skeleton-${index}`} style={{ marginBottom: 12 }}>
              <SkeletonBox style={{ height: 100, borderRadius: 12, marginBottom: 8 }} />
              <SkeletonBox style={{ height: 12, width: '72%', marginBottom: 6 }} />
              <SkeletonBox style={{ height: 12, width: '55%', marginBottom: 6 }} />
