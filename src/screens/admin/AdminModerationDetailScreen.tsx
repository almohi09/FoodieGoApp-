...
import {
  ...
  ReportedItem,
  ApprovalQueueItem,
} from '../../api/adminModerationService';
...
export const AdminModerationDetailScreen: React.FC = () => {
  ...
  const handleDismiss = async () => {
    if (!notes.trim()) {
      showToast({ type: 'info', message: 'Please provide notes for dismissing this item.', });
      return;
    }
    ...
    if (isReported) {
      ...
      if (result.success) {
        ...
        showToast({ type: 'success', message: 'Report has been dismissed.' });
        navigation.goBack();
      } else {
        await logAudit(
          'dismiss_report',
        ...
        showToast({ type: 'error', message: result.error || 'Failed to dismiss report' });
      }
    } else {
      ...
      if (result.success) {
        ...
        showToast({ type: 'success', message: 'Approval request has been rejected.' });
        navigation.goBack();
      } else {
        await logAudit(
          'reject_approval',
        ...
        showToast({ type: 'error', message: result.error || 'Failed to reject approval' });
      }
    }
  ...
  const handleApprove = async () => {
    ...
    if (isReported) {
      ...
      if (result.success) {
        ...
        showToast({ type: 'success', message: 'Report has been actioned.' });
        navigation.goBack();
      } else {
        await logAudit(
          'action_report',
        ...
        showToast({ type: 'error', message: result.error || 'Failed to action report' });
      }
    } else {
      ...
      if (result.success) {
        ...
        showToast({ type: 'success', message: 'Item has been approved.' });
        navigation.goBack();
      } else {
        await logAudit('approve_item', item.type, item.id, false, result.error);
        showToast({ type: 'error', message: result.error || 'Failed to approve item' });
      }
    }
    setActionLoading(false);
  };
  ...
  const handleTakeAction = async (
    ...
    if (!actionReason.trim()) {
      showToast({ type: 'info', message: 'Please provide a reason for this action.', });
      return;
    }
    setActionLoading(true);
    ...
    if (result.success) {
      ...
      showToast({ type: 'success', message: `Action "${action}" has been taken.` });
      navigation.goBack();
    } else {
      await logAudit(
        `take_action_${action}`,
      ...
      showToast({ type: 'error', message: result.error || 'Failed to take action' });
    }
    setActionLoading(false);
  };
